    ...
    <div className="min-h-screen bg-[#FDFBF7] text-[#3D3A37] font-sans antialiased selection:bg-[#EAE0D5]">
        ...
        <div className="bg-gradient-to-br from-[#F7F2EC] via-[#F3ECE5] to-[#EAE0D5] rounded-3xl p-6 md:p-12 border border-[#E0D5C7] shadow-sm flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="w-full md:w-1/2 space-y-4 text-center md:text-left">
            ...
            <div className="pt-2 flex flex-col sm:flex-row gap-3 justify-center md:justify-start">
              <a 
                href="#koleksi" 
                className="inline-flex items-center justify-center gap-2 bg-[#6B705C] hover:bg-[#585C4B] text-white px-6 py-3.5 rounded-full font-medium shadow-sm transition-all text-sm tracking-wide"
              >
          ...
          <div className="w-full md:w-1/2">
            <div className="relative rounded-2xl overflow-hidden shadow-lg aspect-[16/9] md:aspect-[4/3] bg-[#E8E1D9] border-2 border-white">
              <img 
                ...
                alt="Korean Cotton Boutique Showcase" 
      ...
      <section id="koleksi" className="px-5 py-10 max-w-6xl mx-auto">
        <div className="text-center max-w-xl mx-auto mb-10 space-y-2">
          ...
          <p className="text-xs md:text-sm text-[#7F836F]">
            Setiap corak dipilih teliti dengan kualiti benang import Korea yang selesa dan lembut dipakai.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {products.map((item) => (
              <div 
                key={item.id} 
                className="group bg-white rounded-2xl p-3 border border-[#EAE3DA] shadow-sm hover:shadow-md transition-all flex flex-col justify-between"
              >
                <div>
                  ...
                  <h4 className="font-medium text-sm md:text-base text-[#2F3E46] line-clamp-1">
                    {item.name}
                  </h4>
                  
                  <p className="text-xs text-[#8F9489] line-clamp-2 mt-1">
                    {item.description}
                  </p>
                </div>

                <div className="pt-3 mt-2 border-t border-[#F2ECE4] flex items-center justify-between">
                  <div>
                    <span className="text-[10px] text-[#A5A58D] block">Harga / meter</span>
                    <span className="text-sm md:text-base font-semibold text-[#5B4636]">
                      RM {Number(item.price).toFixed(2)}
                    </span>
                  </div>

                  <button className="bg-[#6B705C] hover:bg-[#585C4B] text-white text-xs px-3 py-2 rounded-xl flex items-center gap-1 shadow-sm transition-all">
                    <ShoppingBag size={14} />
                    <span>Beli</span>
                  </button>
                </div>
